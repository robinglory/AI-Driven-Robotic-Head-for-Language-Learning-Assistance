...
class VADRecorder:
    ...
    def __init__(self, sample_rate=16000, frame_ms=30, vad_aggr=3,
        ...
        self.energy_calib_ms = energy_calib_ms
        ...
        def _cb(indata, frames, time_info, status):
            ...
            if energy_thr is None and len(calib_vals) < calib_frames:
                ...
                if len(calib_vals) == calib_frames:
                    ...
                    thr = max(self.energy_min, min(self.energy_max, base * self.energy_margin))
                    energy_thr = thr
            ...
            if speech:
                ...
                if voiced:
                    trailing = min(silence_frames_needed, trailing + 1)
...
class LLMHandler:
    ...
    def _is_quota_or_auth_error(self, exc_msg: str) -> bool:
        m = exc_msg.lower()
        if any(w in m for w in ["429", "too many requests", "rate"]):
            return True
        if any(w in m for w in ["401", "403", "unauthorized", "forbidden", "invalid api key"]):
            return True
