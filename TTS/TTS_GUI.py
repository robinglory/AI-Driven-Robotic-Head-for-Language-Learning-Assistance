...
"""
GUI-only improvements:
- Uses a Notebook with two tabs: “Synthesize” and “FAQ”.
- Clear visual grouping, consistent paddings, bigger touch targets.
- Primary actions row (Generate, Stop, Save, Export) always visible at top.
- Inline helper text and tooltips for each control.
- Model/Status bars improved; “Quick tests” compacted.
- Non-functional “Reset sliders to sensible defaults” for new users.
- No changes to synthesis logic or behavior; same variable names & methods.
"""
...
class Tooltip:
    ...
    def _show(self):
        ...
        frame.pack()
        label = ttk.Label(
            frame, text=self.text, justify=tk.LEFT, wraplength=self.wraplength, style="Tooltip.TLabel"
        )
        label.pack()
...
class PiperTTSPlayer:
    def __init__(self, root):
        ...
        self.add_breaths_var = tk.BooleanVar(value=False)  # placeholder (no functional change)
    ...
    # ---------------- UI ----------------
    def setup_ui(self):
        # Top-level: toolbar + notebook + statusbar
        container = ttk.Frame(self.root, padding=6)
        container.pack(fill=tk.BOTH, expand=True)

        # ===== Toolbar (primary actions always visible) =====
        ...
        nb.pack(fill=tk.BOTH, expand=True, pady=(4, 4))

        # ---- Tab 1: Synthesize ----
        synth_tab = ttk.Frame(nb)
        nb.add(synth_tab, text="Synthesize")
        ...
        b2 = ttk.Button(model_f, text="Reload", command=self.load_voice)
        b2.grid(row=0, column=3)

        Tooltip(entry, "Path to your Piper .onnx voice model file (female voice for Lingo).")
        Tooltip(b2, "Reload the currently selected model.")
        ...
        Tooltip(vol_scale, "Output loudness. 1.0 = default. Increase carefully to avoid clipping.")
        ...
        Tooltip(sp_scale, "Speaking rate. <1.0 = faster; >1.0 = slower. Try 0.9–1.1 for natural pace.")
        ...
        Tooltip(ns_scale, "Pronunciation randomness. Higher = more expressive but can be unstable.")
        ...
        Tooltip(nsw_scale, "Controls intonation/pauses. Moderate values sound most natural (0.7–1.0).")

        # Normalize + Preset
        self.normalize_var = self.normalize_var  # unchanged var
        ttk.Checkbutton(s, text="Normalize audio", variable=self.normalize_var).grid(row=4, column=0, sticky=tk.W, padx=2, pady=(6,2))
        Tooltip(s, "Normalize audio", "Keeps output volume consistent; disable if you do your own mixing.")

        ttk.Label(s, text="Emotion preset").grid(row=4, column=1, sticky=tk.E, padx=(12,2))
        self.preset = tk.StringVar(value="Neutral")
        preset_cb = ttk.Combobox(s, textvariable=self.preset, values=list(self.emotion_presets.keys()), state="readonly")
        preset_cb.grid(row=4, column=2, sticky=tk.W, padx=(2, 6))
        Tooltip(preset_cb, "Quick style starter. You can still tweak sliders after selecting.")
        ...
        Tooltip(pm_scale, "Scales the silence between sentences. Match to your head/jaw movement.")

        # Reset button (UI only)
        def reset_sliders():
            self.vol.set(1.0)
            self.speed.set(1.0)
            self.noise.set(0.667)
            self.noise_w.set(0.8)
            self.pause_mult.set(1.0)
            self.preset.set("Neutral")
        ttk.Button(s, text="Reset to defaults", command=reset_sliders).grid(row=5, column=0, columnspan=6, sticky="e", pady=(6, 2))
        ...
        # ---- Tab 2: FAQ ----
        faq_tab = ttk.Frame(nb)
        nb.add(faq_tab, text="FAQ & Tips")

        faq_text = tk.Text(faq_tab, wrap=tk.WORD, height=20)
        faq_text.pack(fill=tk.BOTH, expand=True, padx=8, pady=8)

        faq_content = (
            "FAQ & Tips (Human-friendly explanations)\n"
            "----------------------------------------\n\n"
            "• Volume: Overall loudness. If you hear distortion, lower this or disable Normalize.\n\n"
            "• Base Speed (length_scale): Controls speaking rate. Lower than 1.0 = faster. "
            "Try 0.9–1.1 for natural conversation. Extreme values can sound choppy or sleepy.\n\n"
            "• Voice Variation (noise_scale): Adds variety to pronunciation. Higher adds personality "
            "but can cause unstable or unclear words. Start ~0.6–0.9.\n\n"
            "• Speaking Style (noise_w_scale): Affects intonation and phrasing. Moderate values (0.7–1.0) "
            "sound most natural. Too high may sound erratic.\n\n"
            "• Normalize audio: Keeps loudness consistent across lines. Turn off if you prefer raw output "
            "for later mixing.\n\n"
            "• Emotion preset: Quick starting points (Neutral, Friendly, Calm, Energetic, Whispery). "
            "You can still adjust sliders afterward.\n\n"
            "• Pause multiplier: Scales silence between sentences. Increase if your robot jaw/neck needs "
            "more time to reset.\n\n"
            "• Caching: The app reuses identical text+settings to save time. If you changed sliders but hear "
            "the old audio, regenerate after making a small text change or clear the cache folder.\n\n"
            "• Common Issues:\n"
            "  - ‘Playback stopped’ too soon: Ensure audio finished generating; try shorter sentences.\n"
            "  - First run lag: Normal; model warm-up. It’s reduced after the first line.\n"
            "  - Robotic tone: Add commas/periods; try preset=Calm and set speed ~1.05; tune noise_scale 0.6–0.8.\n"
            "  - Jaw sync: Use Export Jaw Envelope (.json). Each value is a normalized (0–1) mouth openness every 10ms.\n\n"
            "• Writing for speech: Prefer short sentences, write numbers as words (“twenty three”), and add punctuation "
            "to guide rhythm.\n\n"
            "• Safety: Keep volume moderate to avoid speaker clipping. Consider a limiter in your audio chain.\n"
        )
    ...
    def _add_val_label(self, parent, var, r, c):
        ...
        # live updates whenever slider changes
